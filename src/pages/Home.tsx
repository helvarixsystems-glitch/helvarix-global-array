...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function boot() {
      try {
        ...
        const tasks: Promise<unknown>[] = [loadRecent()];

        if (uid) {
          tasks.push(
            (async () => {
              loadedProfile = await loadProfile(uid);
            })(),
            loadUserSubmissionCount(uid),
            loadCampaignProgress(uid)
          );
        } else {
          tasks.push(loadCampaignsOnly());
        }

        await Promise.all(tasks);
        if (!alive) return;

        await loadEarthSector(loadedProfile);
      } catch (err) {
  ...
  async function loadCampaignsOnly() {
    const data = await cachedQuery("campaigns-active", async () => {
      const { data, error } = await supabase
        .from("campaigns")
        .select(
  ...
  async function loadCampaignProgress(uid: string) {
    ...
    const rows = ((data ?? []) as CampaignProgressRpcRow[]).filter(Boolean);

    const nextProgress: Record<string, number> = {};
    const nextCampaigns: CampaignRow[] = rows.map((row) => {
      nextProgress[row.id] = clamp01(Number(row.progress ?? 0));
  ...
  async function loadEarthSector(profileOverride?: ProfileRow | null) {
    setEarthErr(null);
    setEarthBusy(true);
    ...
    try {
      ...
      const message =
        e instanceof Error ? e.message : "Failed to load earth sector telemetry.";
      setEarthErr(message);
      setEarth(null);
    } finally {
      setEarthBusy(false);
            ...
            <div className="metricRow">
              ...
              <div className="metricCard">
                ...
                <div className="metricRight">
                  <Chip tone={kpTone}>{earth?.kpLabel ?? "UNKNOWN"}</Chip>
                </div>
