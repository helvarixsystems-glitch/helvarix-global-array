...
export const onRequestPost: PagesFunction = async (context) => {
  try {
    ...
    const stripePriceId = String(context.env.STRIPE_PRICE_ID || "").trim();
    ...
    if (!stripePriceId) return json({ error: "Missing STRIPE_PRICE_ID" }, 500);
    ...
    if (!stripeRes.ok) {
      return json(
        {
          error: stripeText || "Stripe session creation failed",
        },
        500
      );
    }

    const stripeData = JSON.parse(stripeText) as { id?: string; url?: string };
